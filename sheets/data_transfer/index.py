...
def merge_coded_data(story_name, coding):
    story_name = settings.BASE_DIR + '/sheets/data_transfer/data/' + f"{story_name}.xlsx"
    people = json.loads(get_people(story_name)[coding].to_json(orient ='records'))
    journalists = json.loads(get_journalist(story_name)[coding].to_json(orient ='records'))
    sheets = json.loads(get_sheet(story_name)[coding].to_json(orient ='records'))

    for sheet in sheets:
        sheet['people'], sheet['journalists'] = [], []
        for person in people:
            if person['story_label'] == sheet['story_label']:
                sheet['people'].append(person)
        for journalist in journalists:
            if journalist['story_label'] == sheet['story_label']:
                sheet['journalists'].append(journalist)
