...
class XLSXReportBuilder:
    ...
    def ws_05(self, ws, gen_dataset=False):
        ...
        for media_type in counts_list:
            for medium in media_type:
                ...
                overall_column += 4
        if gen_dataset:
            tabulate_dataset("ws_05", ["Topic", "Gender", "Medium", "Year", "Count"], counts_list, ws_05_dataset)
    ...
    def ws_06(self, ws, gen_dataset=False):
        ...
        for media_types, models in PERSON_MEDIA_GROUPS:
            ...
            c = ws.dim_colmax + 2
            if gen_dataset:
                tabulate_dataset("ws_06", ["Region", "Topic", "Gender", "Year", "Count"], secondary_counts, ws_06_dataset)
    ...
    def ws_09(self, ws, gen_dataset=False):
        ...
        if gen_dataset:
            tabulate_dataset("ws_09", ["Topic", "Gender", "Year", "Count"], counts, ws_09_dataset)
    ...
    def ws_15(self, ws, gen_dataset=False):
        ...
        if gen_dataset:
            tabulate_dataset("ws_15", ["Function", "Gender", "Year", "Count"], counts, ws_15_dataset)
    ...
    def ws_28b(self, ws, gen_dataset=False):
        ...
        for media_type, model in journalist_models.items():
            ...
            if self.report_type == 'country':
                ...
                if gen_dataset:
                    tabulate_dataset("ws_28b", ["Region", "Medium", "Gender", "Year", "Count"], secondary_counts, ws_28b_dataset, medium=media_type, regions=dict(self.countries))
            else:
                for journo_type, role_ids in reporter:
                    counts = Counter()
                    region = model.sheet_name() + '__country_region__region'
                ...
                write_row_headings = False
                if gen_dataset:
                    tabulate_dataset("ws_28b", ["Region", "Medium", "Gender", "Year", "Count"], secondary_counts, ws_28b_dataset, medium=media_type, regions=self.all_regions)    
    ...
    def ws_28c(self, ws, gen_dataset=False):
        ...
        for media_type, model in broadcast_journalist_models.items():
            ...
            if self.report_type == 'country':
                ...
                write_row_headings = False
                if gen_dataset:
                    tabulate_dataset("ws_28c", ["Region", "Medium", "Gender", "Year", "Count"], secondary_counts, ws_28c_dataset, medium=media_type, regions=dict(self.countries))
            else:
                for journo_type, role_ids in presenter:
                    counts = Counter()
                    region = model.sheet_name() + '__country_region__region'
                ...
                write_row_headings = False
                if gen_dataset:
                    tabulate_dataset("ws_28c", ["Region", "Medium", "Gender", "Year", "Count"], secondary_counts, ws_28c_dataset, medium=media_type, regions=self.all_regions)
    ...
    def ws_30(self, ws, gen_dataset=False):
        ...
        if gen_dataset:
            tabulate_dataset("ws_30", ["Region", "Topic", "Gender", "Year", "Count"], secondary_counts, ws_30_dataset)    
    ...
    def ws_38(self, ws, gen_dataset=False):
        ...
        self.write_overall_value(ws, value, total, overall_column, overall_row, write_overall=True)     
        if gen_dataset:
            tabulate_dataset("ws_38", ["Topic", "Answer", "Year", "Count"], counts, ws_38_dataset)    
    ...
    def ws_41(self, ws, gen_dataset=False):
        ...
        if gen_dataset:
            tabulate_dataset("ws_41", ["Topic", "Answer", "Year", "Count"], counts, ws_41_dataset)    
    ...
    def ws_47(self, ws, gen_dataset=False):
        ...
        self.write_overall_value(ws, value, total, overall_column, overall_row, write_overall=True)
        if gen_dataset:
            tabulate_dataset("ws_47", ["Topic", "Answer", "Year", "Count"], counts, ws_47_dataset)
    ...
    def ws_48(self, ws, gen_dataset=False):
        ...
        for gender_id, gender in self.male_female:
            ...
            secondary_counts[gender] = counts
            if gen_dataset:
                tabulate_dataset("ws_48", ["Topic", "Gender", "Answer", "Year", "Count"], counts, ws_48_dataset, gender=gender_id)
    ...
    def ws_83(self, ws, gen_dataset=False):
        ...
        self.tabulate_secondary_cols(ws, secondary_counts, GENDER, self.all_regions, row_perc=True)
        if gen_dataset:
            tabulate_dataset("ws_83", ["Topic", "Region", "Gender", "Year", "Count"], secondary_counts, ws_83_dataset, regions=self.all_regions)
    ...
    def ws_85(self, ws, gen_dataset=False):
        ...
        if gen_dataset:
            tabulate_dataset("ws_85", ["Region", "Function", "Year", "Count"], counts, ws_85_dataset, regions=self.all_regions)
    ...
    def ws_92(self, ws, gen_dataset=False):
        ...
        for region_id, region in self.all_regions:
            ...
            if gen_dataset:
                tabulate_dataset("ws_92", ["Region", "Topic", "Answer", "Year", "Count"], counts, ws_92_dataset, region=region)
    ...
    def ws_93(self, ws, gen_dataset=False):
        ...
        for region_id, region in self.all_regions:
            ...
            r += len(YESNO)
            if gen_dataset:
                tabulate_dataset("ws_93", ["Region", "Topic", "Answer", "Year", "Count"], counts, ws_93_dataset, region=region)
    ...
    def ws_97(self, ws, gen_dataset=False):
        ...
        if gen_dataset:
            tabulate_dataset("ws_97", ["Region", "Topic", "Agree/Disagree", "Year", "Count"], secondary_counts, ws_97_dataset, regions=self.all_regions)
    ...
    def ws_100(self, ws, gen_dataset=False):
        ...
        if gen_dataset:
            tabulate_dataset("ws_100", ["Topic", "Medium", "Yes/No", "Year", "Count"], secondary_counts, ws_100_dataset)
    ...
    def ws_101(self, ws, gen_dataset=False):
        ...
        if gen_dataset:
            tabulate_dataset("ws_101", ["Topic", "Gender", "Year", "Count"], counts, ws_101_dataset)
    ...
    def ws_102(self, ws, gen_dataset=False):
        ...
        if gen_dataset:
            tabulate_dataset("ws_102", ["Topic", "Agree/Disagree", "Year", "Count"], counts, ws_102_dataset)
    ...
    def ws_104(self, ws, gen_dataset=False):
        ...
        for major_topic, topic_ids in GROUP_TOPICS_MAP.items():
            ...
            r += len(GENDER)
            if gen_dataset:
                tabulate_dataset("ws_104", ["Topic", "Gender", "Function", "Year", "Count"], counts, ws_104_dataset, topic=major_topic_name)
