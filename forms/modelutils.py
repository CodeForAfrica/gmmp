...
field_about_women = lambda x, y: models.CharField(max_length=1, choices=YESNO, verbose_name=_('(%s) Is the %s about a particular woman or group of women?' % (x, force_text(y))))
field_inequality_women = lambda x, y: models.PositiveIntegerField(choices=AGREE_DISAGREE, verbose_name=_('(%s) This %s clearly highlights issues of inequality between women and men' % (x, force_text(y))))
field_stereotypes = lambda x, y: models.PositiveIntegerField(choices=AGREE_DISAGREE, verbose_name=_('(%s) Stereotypes') % x, help_text=_('This %s clearly challenges gender stereotypes' % force_text(y)))
...
field_victim_of = lambda x: models.PositiveIntegerField(choices=VICTIM_OF, verbose_name=_('(%s) The story identifies the person as a victim of:' % x))
...
field_survivor_of = lambda x: models.PositiveIntegerField(choices=SURVIVOR_OF, verbose_name=_('(%s) The story identifies the person as a survivor of:' % x))
